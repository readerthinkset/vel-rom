"""
VELOCITY ROMANIAN - Unified Social Media Upload Script
"""
...
def get_latest_reel():
    ...
    return {"video_path": str(latest), "metadata": meta, "category": meta.get("category_english", "Romanian Learning"), "phrases": meta.get("phrases", [])}


def generate_caption(phrases, category, platform="facebook"):
    base = [f"Learn Romanian with VELOCITY ROMANIAN!", "", f"Category: {category}", "", f"Master Romanian one phrase at a time! Today's {category} lesson:", ""]
    for i, p in enumerate(phrases[:5], 0):
        base.append(f"{i+1}. {p['english']}")
        base.append(f"   {p.get('romanian', '')}")
        base.append(f"   [{p.get('transliteration', '')}]")
        base.append("")
    base.extend(["Tip: Repeat each phrase out loud 3 times!", "Like this video if you learned something new!", "Comment your favorite phrase below!", f"Follow for daily Romanian lessons!", ""])
    base.extend([f"#learnromanian", f"#romanianlessons", f"#romanianforbeginners", "#languagelearning", f"#romanianvocabulary", f"#velocityromanian", f"#dailyromanian", f"#romanian", "#learnlanguages"])
    return "\n".join(base)
...
def upload_to_all_platforms(video_path, caption, category, phrases=None):
    ...
    print("\n" + "="*80)
    print(f"VELOCITY ROMANIAN - MULTI-PLATFORM UPLOAD")
    print("="*80)
        ...
        if func:
            try:
                if pname == "youtube":
                    ...
                    r = func(video_path=video_path, description=caption, title=f"Romanian: {category}")
                elif pname == "telegram":
...
def main():
    print("\n" + "="*80)
    print(f"VELOCITY ROMANIAN - AUTOMATED UPLOAD")
    print("="*80)
